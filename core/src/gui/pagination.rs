...
impl Pagination {
  pub fn new(total: u64, skip: Option<u64>, limit: Option<u64>, max_pages: Option<u64>) -> Self {
    ...
    let max_pages = max_pages.unwrap_or(10).min(total_pages).min(20);
...
fn add_btn(ui: &mut Ui, enabled: bool, text: impl Into<String>, min_size: Vec2) -> Response {
  ui.dx_button_sized_enabled(
    enabled, 
    text,
    16.0,
    DX_Button::Standard,
...
fn add_num_btn(ui: &mut Ui, active: bool, text: impl Into<String>, min_size: Vec2) -> Response {
  ui.dx_button_sized_enabled(
    !active,
    text,
    16.0,
